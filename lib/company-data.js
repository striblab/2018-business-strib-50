/**
 * Get company data from Data Drop database.
 */
...
function fetchData(options = {}) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Write out
      if (options.output) {
        fs.writeFileSync(options.output, JSON.stringify(_.take(companies, 50)));
        ...
        //               : undefined,
        //           footnotes: c.finances[options.publishYear]
        //             ? c.finances[options.publishYear].footnotes
        //             : null
        //         };
        //       })
        //     )
...
function getQuery(query, data) {
  ...
  queries.officers = `
    ...
    FROM
      Officers AS o
        INNER JOIN Officer_Salaries AS s
          ON o.ID = s.OfficerID
    WHERE
      (
        o.Title IS NOT NULL
