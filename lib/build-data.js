...
async function remoteSource(url, options = {}) {
  options.url = url;
  options.ttl = options.ttl || 1000 * 60 * 10;
  ...
  return new Promise((resolve, reject) => {
    request(options, (error, response, body) => {
      ...
      // Handle JSON or CSV
      if (options.type === 'json' || url.match(/json/)) {
        return resolve(JSON.parse(body));
      }
      else if (options.type === 'csv' || url.match(/csv/)) {
...
module.exports = async (sources, cwd = path.join(__dirname, '..')) => {
  ...
  for (let si in sources) {
    ...
    if (_.isFunction(options.transform)) {
      converted[si] = options.transform(converted[si], options);
    }
  }
